...
class Volume {
  ...
  constructor(
    ...
    this.URLs = {};
    this.imgStoredMap =
      imgStoredMap === null ? this.getImgStoredMap(mokuroData) : imgStoredMap;
  ...
  async delete() {
    await delMany(
      this.mokuroData.pages.map((pageJSON) =>
        this.getImgKey(pageJSON.img_path),
      ),
    );
...
class Title {
  ...
  async delete() {
    await Promise.all(
      Object.values(this.volumes).map(async (volume) => {
        await volume.delete();
      }),
    );
