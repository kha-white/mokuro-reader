...
export function updatePage(new_page_idx) {
  ...
  if (
    ...
    volumeState.page_idx < window.numPages - 1 &&
    !isPageFirstOfPair(volumeState.page_idx + 1)
  ) {
  ...
  let page2_txt =
    volumeState.page2_idx >= 0 ? "," + (volumeState.page2_idx + 1) : "";
  document.getElementById("pageIdxDisplay").innerHTML =
    volumeState.page_idx + 1 + page2_txt + "/" + window.numPages;

  setVolumePageState(
    loadedVolume.id,
    Math.max(volumeState.page_idx, volumeState.page2_idx) + 1,
    window.numPages,
  );
  setZoomMinMax();
  zoomDefault();
